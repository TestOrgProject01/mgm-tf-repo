...
public class AuditTrailHandlerTest {
	@Mock  
	 AuditTrailQueueTrigger auditTrailQueueTrigger;
	@InjectMocks
	...
	void testExcute() {
       ...
       doNothing().when(logger).info(anyString());
		ArgumentCaptor<TriggerRequest> triggerRequest = ArgumentCaptor.forClass(TriggerRequest.class);
		auditTrailHandler.execute(message, context);
		verify(auditTrailQueueTrigger, times(1)).apply(triggerRequest.capture());
	}
